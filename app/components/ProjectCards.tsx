        ...
        >
            ...
            >
                <div className="order-2 pb-6 lg:h-full  h-[55%] px-5 lg:pl-10 sm:pr-2 lg:pt-6 pt-5 lg:max-w-[52%] w-full flex flex-col lg:group-even:ml-[29rem] group-even:ml-[0rem] lg:group-odd:mr-[29rem] group-odd:mr-[0rem]">
                    <h3 className="text-2xl font-semibold mb-2 lg:text-left text-center">
                        {title}
                    </h3>
                    <p className=" leading-relaxed sm:text-[1rem] text-[0.8rem] text-gray-700">
                        {description}
                    </p>
                    <div className="mt-auto flex lg:flex-col flex-row justify-between">
                        <div className="lg:w-full w-[50%]  md:block hidden">
                            <h4 className="text-xl font-semibold mb-1 lg:text-left text-center">
                                Features
                            </h4>
                            <ul className="flex flex-col flex-wrap max-h-[5rem] ml-4  gap-x-5">
                                {features.map((feature, index) => (
                                    <li
                                        className="ml-1 leading-relaxed text-gray-800 list-disc "
                                        key={index}
                                    >
                                        {feature}
                                    </li>
                        ...
                        <div className=" lg:w-full  md:w-[50%] w-full  md:pb-2 pb-4">
                            <h4 className="text-xl font-semibold md:mb-1 mb-3 lg:text-left text-center">
                                Technologies
                            </h4>
                            <ul className="flex flex-row  flex-wrap lg:justify-start justify-center lg:mb-4 my-auto gap-2">
                                {tags.map((skill, index) => (
                                    <li
                                        key={index}
                                        className="bg-primary/[0.8] px-3 py-1 md:text-[0.9rem] text-[0.8rem] font-semibold uppercase tracking-wider text-white rounded-full"
                                    >
                                        {skill}
                                    </li>
                                ))}
                    ...
                    <div className="flex sm:gap-4 gap-2 mt-auto lg:mx-0 mx-auto mb-2">
                        <a href={demoUrl} target="_blank">
                            <button className="bg-backgroundHero text-white hover:bg-primary hover:border-primary transition border-2 border-white px-2 py-2 color-white flex items-center justify-center gap-2 sm:text-[1rem] text-[0.9rem] sm:w-[10rem] w-[7rem]">
                                <span className="sm:block hidden">{demoButtonText} </span>
                                <FaArrowUpRightFromSquare />
                            </button>
                        </a>
                        <a href={codeUrl} target="_blank">
                            <button className="bg-backgroundHero text-white hover:bg-primary hover:border-primary transition border-2 border-white px-2 py-2 color-white flex items-center justify-center gap-2 sm:text-[1rem] text-[0.9rem] sm:w-[10rem] w-[7rem]">
                                <span className="sm:block hidden"> Source</span>Code
                                <FaGithub />
                            </button>
                        </a>
                    </div>
                ...
                <Image
                    ...
                    alt="My projects"
                    quality={95}
                    className="absolute hidden lg:block  bottom-0 -right-40 lg:w-[38rem] w-[28rem]
                ...
                <div className="lg:hidden block lg:h-[50%]  object-center overflow-hidden sm:h-[45%] min-h-[170px] ">
                    <Image
                        ...
                        alt="My projects"
                        quality={95}
                        className="order-1  w-full object-fill  rounded-t-lg "
                    />
                </div>
            </section>
        </motion.div>
